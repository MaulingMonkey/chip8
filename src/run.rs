...
impl Context {
    ...
    pub fn try_step_single(&mut self) -> bool {
        ...
        impl Decode for Step<'_> {
            type Result = bool;
            #[inline(always)] fn invalid                (&mut self, op: u16)                    -> Self::Result { panic!("invalid instruction: {op:?}") }
            #[inline(always)] fn call_mcs               (&mut self, addr: Addr)                 -> Self::Result { panic!("unsupported mcs call: {addr}") }
            ...
            #[inline(always)] fn add_v_c                (&mut self, vx: V, c: u8)               -> Self::Result { self.0.registers[vx] += c; self.0.step() }
            ...
            #[inline(always)] fn add_v_v                (&mut self, vx: V, vy: V)               -> Self::Result { self.0.registers[vx] += self.0.registers[vy]; self.0.step() }
            #[inline(always)] fn sub_v_v                (&mut self, vx: V, vy: V)               -> Self::Result { self.0.registers[vx] -= self.0.registers[vy]; self.0.step() }
            ...
            #[inline(always)] fn sub_v_v_alt            (&mut self, vx: V, vy: V)               -> Self::Result { self.0.registers[vx] = self.0.registers[vy] - self.0.registers[vx]; self.0.step() }
            ...
            #[inline(always)] fn draw_x_y_h             (&mut self, vx: V, vy: V, h: Nibble)    -> Self::Result { let _ = (vx, vy, h); todo!("draw_sprite NYI") }
            ...
            #[inline(always)] fn set_i_sprite           (&mut self, v: V)                       -> Self::Result { let _ = v; unimplemented!() }
    ...
    fn screen(&mut self) -> &mut ScreenMonochrome64x32 { self.memory.screen_monochrome_64x32_mut() }
}
